...
def parse_args():
    ...
    parser.add_argument('--context', type=int, default=10, help='''one way context value added along with input t, 
    ...
    parser.add_argument("--allow_partial_load", action="store_true",
                        help="Load checkpoint with strict=False (NOT recommended, debug only)")
...
def main():
    ...
    if (args.revised_model):
        ...
        model = AudioGestureLSTMRevised(
            mfcc_channel=args.mfcc_channel,
            context=args.context,
            hidden_size=args.hidden_size,
            n_joint=args.n_joint,
            silence_npy_path=args.silence_npy_path,
            device=device,
